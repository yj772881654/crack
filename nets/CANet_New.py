...
class CANetGenerator(nn.Module):
        def __init__(self, backbone='resnext101', rate=4):
        ...
        def forward(self, x):
            ...
            features = self.backbone(x) #backbone获取特征
            # self.layers == [3, 4, 6, 3]

            # Scale 1
            begin = 0
            end = begin + self.layers[0] - 1

            # print(begin)
            # print(end)

            s1_sem = self.msblock1(features[end])
            # LayerAttention
            s1_sem = self.LABlock_1(features[begin:end], s1_sem)
            ...
            # Scale 2
            begin = self.layers[0] + begin
            end = begin + self.layers[1] - 1
            #
            # print(begin)
            # print(end)

            s2_sem = self.msblock2(features[end])
            # LayerAttention
            # print(type(features[begin:end]))
            s2_sem = self.LABlock_2(features[begin:end], s2_sem)
            ...
            # Scale 3
            begin = self.layers[1] + begin
            end = begin + self.layers[2] - 1
            #
            # print(begin)
            # print(end)
            s3_sem = self.msblock3(features[end])
            # LayerAttention
            s3_sem = self.LABlock_3(features[begin:end], s3_sem)
            ...
            # Scale 4
            begin = self.layers[2] + begin
            end = begin + self.layers[3] - 1

            # print(begin)
            # print(end)

            s4_sem = self.msblock4(features[end])
            # LayerAttention
            s4_sem = self.LABlock_4(features[begin:end], s4_sem)
            ...
            fuse = self.fuse(torch.cat([p1_1, p2_1, p3_1, p4_1, p1_2, p2_2, p3_2, p4_2], dim=1))

            results = [p1_1, p2_1, p3_1, p4_1, p1_2, p2_2, p3_2, p4_2, fuse]
