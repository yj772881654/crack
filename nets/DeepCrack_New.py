...
class LABlock(nn.Module):
    def __init__(self, input_channels,output_channels):
        ...
        self.W_1 = nn.Sequential(
            nn.Conv2d(input_channels, input_channels//2, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(input_channels//2)
        )

        self.W_2 = nn.Sequential(
            nn.Conv2d(input_channels//2, input_channels//2, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(input_channels//2)
        )

        self.psi = nn.Sequential(
            nn.Conv2d(input_channels//2, output_channels, kernel_size=1, stride=1, padding=0, bias=True),
...
class DeepCrackNewGenerator(nn.Module):
        def __init__(self, rate=4):
            super(DeepCrackNewGenerator, self).__init__()
            self.down1 = Down1()
            self.down2 = Down2()
            self.down3 = Down3()
            self.down4 = Down4()
            ...
            self.fuse4 = Fuse(Conv1X1(512 + 512, 64), scale=8)
            self.fuse3 = Fuse(Conv1X1(256 + 256, 64), scale=4)
            self.fuse2 = Fuse(Conv1X1(128 + 128, 64), scale=2)
        ...
        def forward(self, inputs):
            ...
            # print(up5.shape)
            fuse5 = self.fuse5(down_inp=scale5_3, up_inp=up5, size=[inputs.shape[2], inputs.shape[3]],
                               attention=attention5)
            fuse4 = self.fuse4(down_inp=scale4_3, up_inp=up4, size=[inputs.shape[2], inputs.shape[3]],
                               attention=attention4)
            fuse3 = self.fuse3(down_inp=scale3_3, up_inp=up3, size=[inputs.shape[2], inputs.shape[3]],
                               attention=attention3)
            fuse2 = self.fuse2(down_inp=scale2_2, up_inp=up2, size=[inputs.shape[2], inputs.shape[3]],
                               attention=attention2)
            fuse1 = self.fuse1(down_inp=scale1_2, up_inp=up1, size=[inputs.shape[2], inputs.shape[3]],
